...
from lib3d.my_numpy import opencv2opengl, geodesic
...
def get_obj_poses_from_template_level(
    ...
    if return_cam:
        ...
        obj_poses = np.load(obj_poses_path)
        # trans = np.array([0, 0, 1000]).astype(np.float64)
        # poses = np.tile(np.eye(4)[None], (576,1,1))
        # poses[:, :3, :3] = obj_poses
        # poses[:, :3, 3] = np.tile(trans[None], (576,1))
        # np.save(obj_poses_path, poses)
