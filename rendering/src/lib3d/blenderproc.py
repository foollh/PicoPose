...
def render_blender_proc(
    ...
    # obj_id = int(os.path.basename(cad_path).split(".")[0][4:])
    # cad_info = json.load(open(os.path.join(cad_dir, 'models_info.json')))
    # diameter = cad_info[str(obj_id)]['diameter']
    # scale = 1 / diameter
    ...
    for idx_frame, obj_pose in enumerate(obj_poses):
        rgb_save_path = os.path.join(output_dir, "{:06d}.png".format(idx_frame))
        depth_save_path = os.path.join(output_dir, "{:06d}_depth.png".format(idx_frame))
        if os.path.exists(rgb_save_path) and os.path.exists(depth_save_path):
            continue
        # obj.set_scale([scale, scale, scale], idx_frame)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cad_path", default="/cluster/sc_download/bop_datasets/tless/models_cad/obj_000003.ply", help="Path to the model file")
    parser.add_argument("--obj_pose", default="/cluster/sc_download/bop_datasets/templates/tless_normalize/object_poses/000003.npy", help="Path to the model file")
    parser.add_argument(
        "--output_dir", default="/cluster/sc_download/bop_datasets/templates/tless_normalize/000003", help="Path to where the final files will be saved"
    )
    parser.add_argument("--gpus_devices", default="0", help="GPU devices")
    parser.add_argument("--disable_output", default="false", help="Disable output of blender")
    parser.add_argument(
        "--scale_translation", default="true", help="scale translation to meter"
    )
