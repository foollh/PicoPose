...
class Keypoint:
    ...
    def project(self, name, K):
        ...
        return project_points(points, K)
...
class KeyPointSampler:
    def __init__(self, tar_size=224, patch_size=3.5):
        self.tar_size = tar_size
        self.patch_size = patch_size
        self.grid_points = self.init_points2d(patch_size)

    def init_points2d(self, patch_size):
        """
        Initialize the normalized center patch
        """
        x = torch.arange(0, self.tar_size, patch_size).float()
        x += patch_size / 2
        y = torch.arange(0, self.tar_size, patch_size).float()
        y += patch_size / 2
    ...
    def sample_pts(
        ...
        batch_size = src_data.mask.shape[0]
        init_points = self.grid_points.clone().unsqueeze(0).to(src_data.mask.device)
        ...
        for idx in range(batch_size):
            distance_ = torch.cdist(
                reproj_key_pts2d.src[idx].float(), key_pts2d.tar[idx].float()
            )
            ...
            reproj_key_pts2d.src[idx, ~mask_] = -1
            key_pts2d_cropped.src[idx, ~mask_] = -1

        # convert to patch coordinates
        src_patchs = self.convert_to_patch_coordinates(key_pts2d_cropped.src)
        tar_patchs = self.convert_to_patch_coordinates(reproj_key_pts2d.src)
